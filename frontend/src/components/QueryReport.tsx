...
const DEFAULT_REGION_STYLE = { bg: 'bg-zinc-400', text: 'text-zinc-950' };
...
const ContextMap = ({ contextMap }: { contextMap?: ReportContextMap }) => {
    ...
    return (
        <div className="bg-zinc-50 rounded border border-zinc-200 p-3 inline-block">
...
const StructureSection = ({
    ...
    return (
        <div className="mb-4">
            <h3 className="text-sm font-bold text-zinc-700 mb-2">{title} <span className="font-normal text-zinc-500">({items.length})</span></h3>
            ...
            <div className="p-6 max-w-5xl mx-auto bg-white text-black">
                ...
                {data.contextMap && (
                    <div className="break-avoid mb-6">
                        <h2 className="text-lg font-bold text-zinc-800 mb-2 border-b border-zinc-300 pb-1">CONTEXT MAP</h2>
                        <ContextMap contextMap={data.contextMap} />
                    </div>
                )}

                <div className="break-avoid mb-6">
                    <h2 className="text-lg font-bold text-zinc-800 mb-2 border-b border-zinc-300 pb-1">MOLIGO PROVENANCE SCHEMATIC</h2>
                    <MOLigoSchematic
                        light
                ...
                <div className="break-inside-avoid mb-6 grid grid-cols-1 gap-3">
                    <div>
                        ...
                        <div className="text-xs text-zinc-600 mt-0.5 grid grid-cols-2 gap-x-6 gap-y-0.5 whitespace-pre-wrap">
                            ...
                            <span><span className="text-zinc-500">Self-Dimer Strider ΔG{'\t'}</span><V>{fmtDG(extractTopLocalDg(data.idtM1SelfDimer))}</V></span>
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <div className="text-xs text-zinc-600 mt-0.5 grid grid-cols-2 gap-x-6 gap-y-0.5 whitespace-pre-wrap">
                            ...
                            <span><span className="text-zinc-500">Self-Dimer Strider ΔG{'\t'}</span><V>{fmtDG(extractTopLocalDg(data.idtM2SelfDimer))}</V></span>
                        </div>
                    </div>
                </div>
                ...
                {data.tagSeq && (
                    <div className="break-avoid mb-6">
                        ...
                        <p className="font-mono text-sm break-all bg-zinc-50 p-3 rounded border border-zinc-200">{data.tagSeq}</p>
                        {data.tagReg != null && (
                            <p className="text-sm text-zinc-600 mt-1">A{String(data.tagReg).padStart(3, '0')}</p>
                        )}
                    </div>
                )}
                ...
                <div className="break-avoid mb-6">
                    <h2 className="text-lg font-bold text-zinc-800 mb-2 border-b border-zinc-300 pb-1">FLANKING PRIMERS</h2>
                    {data.flankingFwdSeq ? (
                        <div className="mb-3">
                ...
                {data.savedPositions && data.savedPositions.length > 0 && (
                    <div className="break-avoid mb-6">
                        <h2 className="text-lg font-bold text-zinc-800 mb-2 border-b border-zinc-300 pb-1">SAVED POSITIONS</h2>
                        <div className="space-y-3">
